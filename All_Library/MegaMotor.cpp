...
const float kp = 8e-2,ki = 1.3e-5,kd = 5,dt = 1.5,turnCoe = 100;
//l:0 r:1 b:2
...
//  Serial.println(theta);
  ...
  if(0 <= theta && theta < 7){
    if(theta == 6.5){
      ...
      pureMove(moveDire,6000,12);
      return moveDire;
    }
  }
  else if(7 <= theta && theta < 14){
    if(theta == 13.5){
      moveToBall(moveDire-7,2000,8);
      return moveDire-7;
    }
    else if(theta == 13.8){
      moveToBall(moveDire-7,0,1);
      return -1;
    }
    else{
...
void Encoder_Control(float goal[4]){
  ...
  while(!(tf[0] && tf[1] && tf[2] && tf[3])){
    ...
    if (mcp2515.readMessage(&canMsg_IN) == MCP2515::ERROR_OK) {
      ...
      Serial.println(mcp2515.readMessage(&canMsg_IN));
    }
  }
  Serial.println("ok");
  for (int i = 0;i < 4;i++){
